...
whisper_available = False  # سيتم ضبطه على True في حال نجاح تهيئة النموذج
whisper_model = None
...
try:
    ...
    # Check if whisper is available in the global Python environment
    try:
        subprocess.check_call([sys.executable, '-m', 'pip', 'show', 'openai-whisper'])
        logger.info("Whisper package is installed in the system")
        import whisper
        import torch
        ...
        logger.error("Whisper package is not installed in the system")
        whisper_model = None
...
def get_task_status(task_id):
    ...
    try:
        ...
        logger.info(f"Returning task status for {task_id}: {task_info}")
        
        # Check if task is done and has a result
        ...
        # If task is still running, return progress
        return jsonify({
            'status': task_info.get('status', 'processing'),
            'progress': task_info.get('progress', 0),
            'message': task_info.get('message', 'جاري المعالجة...'),
...
def generate_subtitles_with_whisper(video_path, language, task_id=None):
    """Extract audio from video and generate subtitles using Whisper"""
    global whisper_model, whisper_available
    
    # Check if Whisper is available
    if whisper_model is None:
        logger.error("Whisper model not initialized but whisper_available is set to True")
        if task_id:
            tasks[task_id]['status'] = 'failed'
            tasks[task_id]['message'] = 'فشل في تحميل نموذج التعرف على الكلام'
            tasks[task_id]['progress'] = 0
        raise Exception("فشل في تحميل نموذج التعرف على الكلام - Whisper model is None")
...
def download_youtube_video(youtube_url):
    ...
    try:
        ...
        raise Exception("فشل تنزيل الفيديو من يوتيوب")
    ...
    try:
        ...
        try:
            ...
            if not subtitles or len(subtitles.strip()) == 0:
                ...
                raise Exception("لم يتم التعرف على أي كلام في الفيديو")
            ...
            logger.error(f"Error during YouTube video processing: {str(inner_error)}")
            tasks[task_id]['status'] = 'error'
            tasks[task_id]['message'] = f'حدث خطأ: {str(inner_error)}'
            tasks[task_id]['error'] = str(inner_error)
